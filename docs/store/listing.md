...
- **alarms** — "Wakes the service worker when a tab reminder is due so the toolbar badge can show the number of arrived reminders."
- **contextMenus** — "Adds right-click items to send the current tab, tabs from the same site, or tabs to the left/right into SonTab."
